...
CardSelectForm.prototype.setMinMaxListeners = function() {
  var selectForm = this;
  var minBoxes = document.getElementsByClassName('min-option');
  var maxBoxes = document.getElementsByClassName('max-option');
  this.setMinMaxBoxes(minBoxes, maxBoxes);
  this.el.addEventListener('change', function (event) {
    var target = event.target;
    if (Array.prototype.indexOf.call(minBoxes, target) >= 0) {
      selectForm.validateMax(event.target);
    } else if (Array.prototype.indexOf.call(maxBoxes, target) >= 0) {
      selectForm.validateMin(event.target);
    }
  });
};

CardSelectForm.prototype.setMinMaxBoxes = function (minBoxes, maxBoxes) {
  for (var i = 0; i < minBoxes.length; i++) {
    minBoxes[i].maxBox = maxBoxes[i];
    maxBoxes[i].minBox = minBoxes[i];
  }
};

CardSelectForm.prototype.validateMax = function (minBox) {
  var maxBox = minBox.maxBox;
  maxBox.value = (maxBox.value < minBox.value) ? minBox.value : maxBox.value;
};

CardSelectForm.prototype.validateMin = function (maxBox) {
  var minBox = maxBox.minBox;
  minBox.value = (minBox.value > maxBox.value) ? maxBox.value : minBox.value;
};
