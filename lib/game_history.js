...
D.GameHistory = class GameHistory {
  constructor(game) {
    this.kingdomCards = D.GameHistory.convertCards(game.kingdomCards);
    this.eventsAndLandmarks = D.GameHistory.convertCards(game.eventsAndLandmarks);
...
D.NavigationDisplay = class NavigationDisplay {
  constructor(options) {
    this.el = options.el;
  ...
  initializeDisplay() {
    this.forwardButton = D.createDOMElement({
      type: 'button',
      attributes: {
        innerText: 'next set',
        hidden: true,
    ...
    this.backButton = D.createDOMElement({
      type: 'button',
      attributes: {
    ...
    this.navigationMessage = D.createDOMElement({
      type: 'div',
      attributes: {
        hidden: true,
