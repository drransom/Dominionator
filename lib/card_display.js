...
CardDisplay.prototype.createTextSpan = function() {
  ...
  this.textSpanElement.innerHTML = this.selector.constructCardName(this.card);
};

CardDisplay.prototype.createCardAttributes = function() {
  this.el.insertAdjacentHTML('beforeend', this.calculateCostString());
  if (this.card.isPotionCard()) {
    this.el.appendChild(this.potionImg);
  }
  this.el.insertAdjacentHTML('beforeend',
    "<span class='dominionator-set-name'>" + this.card.set + "</span>");
};

CardDisplay.prototype.calculateCostString = function() {
  var numCost = this.card.getNumCost();
  if (numCost || !this.card.isPotionCard()) {
    return "<span class='dominionator-card-cost-num'>" + numCost + "</span>";
  } else {
    return "";
  }
};
