...
CardDisplay.prototype.hide = function(reverse) {
  if (reverse) {
    this.slideLeft();
  } else {
    this.slideRight();
...
CardDisplay.prototype.unhide = function(reverse) {
  var node = this.node;
  if (reverse) {
    this.teleportRight();
  }
...
CardDisplay.prototype.slideRight = function() {
  ...
  //   display.teleportLeft();
  // };
  // D.addTransitionEvent(this.node,);
};
...
CardDisplay.prototype.setSlideListener = function() {
  var node = this.node;
  var callback = function() {
    D.removeClass(node, "slideLeft");
    D.removeClass(node, "slideRight");
  };
  D.addTransitionEvent(node, callback);
};
