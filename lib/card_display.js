...
D.CardDisplay = function(options) {
  ...
  this.node = this.constructElement();
};
...
CardDisplay.prototype.constructElement = function() {
  ...
  textSpanElement.setAttribute('id', this.card.nameIdHtml());
  textSpanElement.appendChild(document.createTextNode(cardName));
  var cardAttributesString = this.card.costString() + " " + this.card.set;
  var cardAttributesElement = document.createTextNode(cardAttributesString);
  var vetoButtonElement = this.createVetoButton();
  li.appendChild(textSpanElement);
  li.appendChild(cardAttributesElement);
  li.appendChild(vetoButtonElement);
  return li;
};
...
CardDisplay.prototype.createVetoButton = function() {
  ...
  buttonElement.setAttribute('id', this.card.id);
