...
class Game {
  ...
  selectnCardsFromSet(setName, numCards, chooseEvents) {
    ...
    while (currentSet.length > 0 &&
      ...
      } else if (this.isValidLandscape(card, chooseEvents) &&
          (!otherCards.length ||
  ...
  trySelectOneCard(chooseEvents) {
    ...
    if (this.isValidLandscape(card, chooseEvents)) {
  ...
  isValidLandscape(card, chooseEvents) {
    if (!card.isLandscape() || !chooseEvents) {
      return false;
    }
    if (this.landscapes.length >= 2 || this.landscapes.indexOf(card) >= 0) {
      return false;
    }
    if (card.isWay
          && this.landscapes.length === 1
          && this.landscapes[0].isWay) {
      return false;
    }
    if (this.isVetoedCard(card)) {
      return false;
    }
    if (Game.countBySet(this.allCardsInGame(), card.set) >= this.maxBySet[card.set]) {
      return false;
    }
    return true;
  }
