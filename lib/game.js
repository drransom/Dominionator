...
class Game {
  ...
  veto(card) {
    ...
    } else if ((index = this.landscapes.indexOf(card)) >= 0) {
  ...
  vetoAndReplaceEventByIndex(index) {
    this.vetoCardByIndex(index, this.landscapes);
    this.addNewEvent();
  }

  addNewEvent() {
    this.addNewEventOrLandmark('events');
    // let newEvent, events = Helpers.where(this.currentCards, {isEvent: true});
    // let otherCards = Helpers.where(this.currentCards, {isEvent: false});
    // while (events.length > 0) {
    //   newEvent = events.pop();
    //   if (this.isValidEvent(newEvent)) {
    //     newEvent.addToSelected({selectedCards: this.landscapes});
    //     break;
    //   }
    // }
  ...
  addNewEventOrLandmark(typeToUse) {
    let newCard, possibleCards, trueOption, falseOption, validationFunction;
    if (typeToUse === 'events') {
    ...
    if (typeToUse) {
      ...
      let otherCards = Helpers.where(this.currentCards, falseOption);
