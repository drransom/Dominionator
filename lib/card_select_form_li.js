...
D.CardSelectLi = class CardSelectLi {
  constructor(setName) {
  ...
  createSelectOption(minOrMax) {
    ...
    let selector = D.createDOMElement({
      type: 'select',
      classes: ['dominionator-option'],
      attributes: { id: D.htmlize(this.setName) + '-' + minOrMax }
    });
    for (let i = 0; i <= 10; i++) {
      let option = D.createDOMElement({
        type: 'option',
  ...
  createIncludeExclude(includeExclude) {
    let id = "dominionator-" + D.htmlize(this.setName) + "-" + includeExclude;
    let buttonLabel = D.createDOMElement({
      type: 'div',
