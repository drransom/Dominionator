...
class Card {
  constructor(object) {
    ...
    this.cost = new Cost(object);
  }

  static constructCard(object) {
    if (object.set === "Alchemy") {
      return new AlchemyCard(object);
    } else {
  ...
  static constructProperty(object, property) {
    ...
    if (currentVal === 'N') {
      ...
      value = Helpers.titleCase(currentVal);
    } else {
      value = currentVal;
    }
    return value;
  ...
  isLandscape() {
    return this.isEvent || this.isLandmark || this.isWay;
  }
...
class AlchemyCard extends Card {
  constructor(object) {
    super(object);
  }
