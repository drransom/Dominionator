...
class GameDisplay {
  ...
  createSectionHeaders() {
    ...
    this.landscapesArea = this.createDomWithHeader({
      elementType: 'section', sectionId: 'dominionator-events-area',
      headerType: 'h1', headerText: 'Landscape Cards',
      hidden: "left", headerId: 'dominionator-events-header'});
    this.landscapesList = this.landscapesArea.appendChild(Helpers.createDOMElement({
      type: 'ul', classes: ['dominionator-list', 'standard-left-margin-padding'] }));
    ...
    this.el.appendChild(this.landscapesArea);
  ...
  displayOneCard(card) {
    if (card.isEvent || card.isLandmark) {
      this.landscapesList.appendChild(card.el());
    } else {
  ...
  updateCardDisplay(options) {
    ...
    if (this.currentGame.landscapes.length > 0) {
      this.displayArea(this.landscapesArea);
    } else {
      this.hideArea(this.landscapesArea);
    }
  ...
  updateVetoDisplay(cardType) {
    let area = ((cardType === "kingdom") ? this.kingdomArea : this.landscapesArea);
