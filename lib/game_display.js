...
GameDisplay.prototype.createSectionHeaders = function () {
  ...
  this.eventsArea = this.createDomWithHeader({
    ...
    headerType: 'h1', headerText: 'Events',
    hidden: "left", headerId: 'dominionator-events-header'});
  this.landmarksArea = this.createDomWithHeader({
    elementType: 'section', sectionId: 'dominionator-landmarks-area',
    headerType: 'h1', headerText: 'Landmarks',
    hidden: "left", headerId: 'dominionator-landmarks-header'});
  this.eventsList = this.eventsArea.appendChild(D.createDOMElement({
    type: 'ul', classes: ['dominionator-list', 'standard-left-margin-padding'] }));
  this.landmarksList = this.landmarksArea.appendChild(D.createDOMElement({
    type: 'ul', classes: ['dominionator-list', 'standard-left-margin-padding'] }));
  ...
  this.el.appendChild(this.landmarksArea);
...
GameDisplay.prototype.displayOneCard = function (card) {
  if (card.isEvent) {
    this.eventsList.appendChild(card.el());
  } else if (card.isLandmark) {
    this.landmarksList.appendChild(card.el());
  } else {
...
GameDisplay.prototype.updateCardDisplay = function (options) {
  ...
  if (this.currentGame.landmarks.length > 0) {
    this.displayArea(this.landmarksArea);
  } else {
    this.hideArea(this.landmarksArea);
  }
