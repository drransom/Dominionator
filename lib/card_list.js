...
CardList.prototype.updateKingdomCards = function () {
  this.currentCards = _.shuffle(this.currentCards);
  if (this.options.minSets) {
    this.selectCardsFromSets(this.options.minSets);
  }
  while (this.kingdomCards.length < 10 && this.currentCards.length > 0) {
    this.trySelectOneCard(this.options);
  }
  this.processSpecialCases(this.options);
};
...
CardList.prototype.selectnCardsFromSet = function (set, numCards) {
  ...
  while (currentKingdom.length > 0 && counter < (numCards - currentNum) &&
    ...
    } else if (this.isValidEvent(card) &&
        (!otherCards.length ||
