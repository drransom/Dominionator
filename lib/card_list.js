...
CardList.prototype.updateKingdomCards = function (options) {
  options = options || {};
  var counter = 0, max = options.max || 10;
  if (options.minSets) {
    this.selectCardsFromSets(options.minSets);
  }
  while (this.kingdomCards.length < max && counter < 100) {
    this.selectOneValidCard(options);
    counter += 1;
    if (counter >= 100) {
...
CardList.prototype.selectnCardsFromSet = function (set, numCards) {
  var currentSet, numInSet = _.where(this.kingdom_cards, {set: set}).length, count = 0;
  if (set === "Adventures") {
    currentSet = _.where(this.sets[set], {isEvent: false });
  } else {
    currentSet = this.sets[set];
  }
  while (numInSet < numCards) {
    newCard = this.selectOneValidCard( {set: currentSet} );
    numInSet = _.where(this.kingdom_cards, {set: set}).length; //can't just increment because of Young Witch issue
  }
};

CardList.prototype.selectOneValidCard = function (options) {
  var card;
  options = options || {};
  if (options.set) {
    card = _.sample(options.set);
  } else {
