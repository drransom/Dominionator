...
var D = Dominionator,  Card = D.Card;
...
Game.prototype.addNewEvent = function() {
  ...
  // var newEvent, events = D.where(this.currentCards, {isEvent: true});
  // var otherCards = D.where(this.currentCards, {isEvent: false});
  ...
  //   }
  // }
  // this.currentCards = D.shuffle(otherCards.concat(events));
};
...
Game.prototype.addNewEventOrLandmark = function(typeToUse) {
  ...
  if (typeToUse) {
    possibleCards = D.where(this.currentCards, trueOption);
    otherCards = D.where(this.currentCards, falseOption);
    ...
    this.currentCards = D.shuffle(otherCards.concat(possibleCards));
  }
};
