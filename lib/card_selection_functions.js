...
Game.prototype.selectnCardsFromSet = function (setName, numCards, chooseEvents) {
  var currentSet, currentKigdomEvents, otherCards, events, card;
...
Game.prototype.numCardsInSet = function (setName) {
  return _.where(this.kingdomCards.concat(this.events), {set: setName}).length;
};
...
Game.prototype.isValidEvent = function (card) {
  return card.isEvent && !this.isVetoedCard(card) &&
         ...
         Game.countBySet(this.kingdomCards.concat(this.events), card.set) < this.max[card.set];
};
